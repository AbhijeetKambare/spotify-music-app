...
public class Album {
    ...
    public void addSongtoAlbum(String title, double duration){
        ...
        if(findSong(s.getTitle())==true){
            System.out.println(title+"Song already present in album");
        }
        else{
            songs.add(s);
            System.out.println("New Song "+title+"has been added");
        }
        return;
    }
        ...
        if(findSong(title)==true){
            for(Song song: songs) {
                if (song.getTitle().equals(title)) {
                    playList.add(song);
                    System.out.println("Song "+title+"has been added to your playlist");
                    break;
                }
            }
        }
    ...
    public void addToPlaylistFromAlbum(int trackNo, LinkedList<Song> playList){
        ...
        if(index>=0 && index<songs.size()){
            System.out.println("Song no "+index+" "+ songs.get(index).getTitle()+" has been added to your playlist");
